...
.. meta::
   :description: C++ move semantics guide covering std::move, std::forward, Rule of Zero/Three/Five, value categories (lvalue, xvalue, prvalue, glvalue, rvalue), universal references, perfect forwarding, noexcept, moved-from state, and common pitfalls.
   :keywords: C++ move semantics, std::move, std::forward, Rule of Five, Rule of Three, Rule of Zero, value categories, lvalue, rvalue, xvalue, prvalue, glvalue, universal reference, forwarding reference, perfect forwarding, noexcept, move constructor, move assignment, move-only, copy constructor, destructor

.. contents:: Table of Contents
    :backlinks: none

Move semantics (introduced in C++11) enable efficient transfer of resources
between objects without deep copying. Where copy operations duplicate state,
move operations *steal* it—leaving the source in a valid but unspecified state.
This distinction transforms what used to be expensive ``O(n)`` deep copies into
``O(1)`` pointer swaps, making value types practical to return from functions
and store in containers.

Understanding move semantics requires four interrelated concepts: the six
**special member functions** that the compiler generates or you write by hand,
the **value categories** (lvalue, rvalue, and their refinements) that determine
which of those functions are called, the **Rule of Zero/Three/Five** for writing
correct resource-owning classes, and **perfect forwarding** (``std::forward``)
for template code that must preserve value categories across function calls.
This chapter covers all four, along with common pitfalls.
...
See :doc:`cpp_basic` for a fuller treatment of template type deduction that
produces these collapsing cases.
...
reference collapsing. The first overload handles lvalue arguments: when ``T``
is an lvalue reference type, ``T&&`` collapses to an lvalue reference. The
second overload handles rvalue arguments and includes a static assertion to
prevent forwarding an rvalue as an lvalue:
